...
from bioalloyrag.data_processing import DataProcessor
...
def main(
    ...
    if data_path:
        ...
        if os.path.exists(vectordb_path) and any(Path(vectordb_path).iterdir()):
            print(
                "A new document path is provided. Clearing the existing knowledge base to create a fresh one."
            )
        ...
        print(
            "No knowledge base found. Please provide a directory of PDF files to create a custom, searchable knowledge base for grounding responses."
        )
        data_path = input(
        ...
        print(
            "Knowledge base found. Proceeding with responses grounded in this existing knowledge base."
        )
        # print(f"Using vector database path: {vectordb_path}")
