...
# and `pip install BioAlloyRAG` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "BioAlloyRAG", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n BioAlloyRAG -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "BioAlloyRAG", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "BioAlloyRAG", "YOUR_CONSOLE_SCRIPT"]
